    ...
    <main className="game-shell flex-1 overflow-hidden bg-[#f8f5ed] dark:bg-[#111310]">
      ...
      <section className="p-3 sm:p-4">
        <div className={`battlefield battle-theme-${battleTheme} relative h-64 overflow-hidden rounded-[28px] border shadow-inner`}>
          ...
          {(state.phase === 'ready' || state.phase === 'won' || state.phase === 'lost') && <div className="absolute inset-0 z-50 flex flex-col items-center justify-center overflow-hidden bg-gradient-to-b from-[#fff3ba]/95 via-[#dff7d8]/95 to-[#8bc982]/95 px-6 text-center text-[#273322] backdrop-blur-[3px] dark:from-[#26351d]/95 dark:via-[#173126]/95 dark:to-[#173c24]/95 dark:text-white"><span className="game-intro-star absolute left-8 top-8 text-2xl text-amber-400">★</span><span className="game-intro-star absolute right-10 top-14 text-lg text-amber-300">✦</span><span className="game-intro-carrot absolute right-6 top-28 text-2xl">🥕</span>{state.phase === 'ready' ? <div className="relative h-28 w-56"><div className="absolute bottom-0 left-4 -rotate-6"><TierBunny tier="길토끼" size={78} /></div><div className="absolute bottom-1 left-1/2 z-10 -translate-x-1/2"><TierBunny tier={tierLabel} size={105} /></div><div className="absolute bottom-0 right-3 rotate-6"><TierBunny tier={unlockedUnits.includes('ninja') ? '들토끼' : '풀토끼'} costume={unlockedUnits.includes('ninja') ? 'ninja' : undefined} size={78} /></div></div> : <div className="rounded-full bg-white/25 p-2 ring-1 ring-white/40"><TierBunny tier={tierLabel} size={88} /></div>}<p className="mt-1 text-[10px] font-black tracking-[0.22em] text-amber-700 dark:text-amber-300">RABBIT GUARDIANS</p><p className="mt-1 text-2xl font-black">{state.phase === 'ready' ? '토끼들아, 출동 준비!' : state.phase === 'won' ? '20스테이지 방어 성공!' : '토끼굴 방어 실패'}</p><p className="mt-1 text-xs font-bold">오늘 최고 {displayedBestStage}스테이지 · 도전 {attemptsUsed}/{MAX_ATTEMPTS_PER_DAY}회</p><p className="mt-0.5 text-[11px] text-current/65">{state.phase === 'ready' ? '기록은 게임이 종료되고 서버 검증이 끝나면 저장돼요.' : resultMessage || `${state.clearedStages}스테이지 기록을 서버에서 검증 중...`}</p><button type="button" disabled={starting || (state.phase === 'ready' && attemptsUsed >= MAX_ATTEMPTS_PER_DAY)} onClick={() => state.phase === 'ready' ? beginRun() : reset()} className="mt-3 rounded-full bg-amber-400 px-7 py-3 text-sm font-black text-gray-950 shadow-xl shadow-emerald-950/20 transition hover:-translate-y-0.5 disabled:opacity-40">{state.phase === 'ready' ? starting ? '도전권 확인 중...' : attemptsUsed >= MAX_ATTEMPTS_PER_DAY ? '오늘 도전 완료' : '수호대 출동!' : '다시 도전'}</button></div>}
        </div>
      </section>
