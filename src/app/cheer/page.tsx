...
const RANK_COLORS = ['text-amber-500', 'text-gray-400', 'text-orange-600', 'text-gray-500', 'text-gray-500'];
const RANK_LABELS = ['1st', '2nd', '3rd', '4th', '5th'];

export default async function CheerPage({ searchParams }: Props) {
  ...
  const [{ items: scoreItems }, { items: popularItems }, reviewsResult] = await Promise.all([
    ...
    supabase
      .from('reviews')
      .select('id, webtoon_id, score, comment, recommend_count, created_at, profiles(nickname, total_recommends), webtoons(title)')
          ...
          {topReviews.length === 0 ? (
            ...
            <ul className="grid gap-3">
              {topReviews.map((review, i) => {
                ...
                const profile = review.profiles as { nickname?: string; total_recommends?: number } | null;
                return (
                  <li key={review.id}>
                    <Link href={`/webtoon/${review.webtoon_id}`} className="block rounded-xl border border-gray-100 bg-white p-3 hover:border-amber-200 dark:border-gray-900 dark:bg-gray-950 dark:hover:border-amber-900">
                      <div className="mb-2 flex items-center justify-between gap-2">
                        <div className="flex items-center gap-2 min-w-0">
                          <span className={`text-[11px] font-black tabular-nums ${RANK_COLORS[i]}`}>{RANK_LABELS[i]}</span>
        ...
        <ul className="grid gap-2">
          {items.map((w, i) => (
            <li key={w.id}>
              ...
              >
                <div className={`text-center text-sm font-black tabular-nums ${RANK_COLORS[i]}`}>{i + 1}</div>
